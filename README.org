...
**NOTE**: The ServiceKey secret doesn't have to be in the same namespace as the OriginIssuer, because it's being
...
below example (that is, `issuer` is the name of your issuer, `issuer-kind: OriginIssuer` and `issuer-group: cert-manager.k8s.cloudflare.com`). 
